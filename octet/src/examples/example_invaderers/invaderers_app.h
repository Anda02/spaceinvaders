	...
	*/
    ...
    void move_ship() {
	  ...
	  if (is_key_down(VK_SPACE)) {
		  //measures when the clock starts
		  std::chrono::time_point<std::chrono::system_clock> start, end;
		  start = std::chrono::system_clock::now();
		  //measures when the clock ends
		  while (is_key_down(VK_SPACE)==true)
		  {
			  std::chrono::time_point<std::chrono::system_clock> start, end;
  ...
  public:
    ...
    void app_init() {
	  ...
	  */
      ...
      sprites[first_border_sprite+0].init(white, 0, -3, 6, 0.1f);
      sprites[first_border_sprite+1].init(white, 0,  3, 6, 0.1f);
      sprites[first_border_sprite+2].init(white, -3, 0, 0.1f, 6);
      sprites[first_border_sprite+3].init(white, 3,  0, 0.1f, 6);
